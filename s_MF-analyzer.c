/*  Program s_MF-analyzer.c. Version: May 22nd, 2007 */

/*  Authors: Antonio Turiel and Oriol Pont         */

/*
    s_MF-analyzer is a simplified, non-NetCDF version of MF-analyzer.
*/
...
/*
    ...
    Multifractal Formalism on that piece of data;
    ...
    execution; type s_MF-analyzer -h for a wider explanation.
*/
...
#define NUMERICAL_RESOLUTION 1e-10 // Maximum allowed numerical resolution
...
float GOODRHO=0.999; // Goodness for singularity analysis regressions
...
/* Resolution is changed, if required  */
   ...
   if (fabs(BLOCK-1.)>NUMERICAL_RESOLUTION)
   {
...
/* If data must be logarithmized, this is the moment for doing so */
   ...
   if (LOGDATA)
   {
    ...
    {
     if (fabs(data.Signal[iy][ix]) > NUMERICAL_RESOLUTION)
      data.Signal[iy][ix] = log( fabs(data.Signal[iy][ix]) );
     else
      data.Signal[iy][ix] = log(NUMERICAL_RESOLUTION);
    }
    }
   }
...
/* Accumulating the percentage of good regression points */

   goodrho_acc += summary.densgood;
   goodrho_count++;
 ...
 if (SAVEDATA)
 {
  ...
  for (in=0;in<NDATA;in++)
  {
   ...
   for (iz=iz0;iz<iz0+dimz;iz++)
   {
    ...
    if (fabs(BLOCK-1.)>NUMERICAL_RESOLUTION) change_resolution(BLOCK,&data);
    if (LOGDATA) for (ip=0;ip<data.dimx*data.dimy;ip++)
    {
     if (fabs(data.p[ip])>NUMERICAL_RESOLUTION) data.p[ip]=log(fabs(data.p[ip]));
     else                                       data.p[ip]=log(NUMERICAL_RESOLUTION);
    }
...
/* Informing about the global goodness of regressions */

 if (goodrho_count > 0)
 {
  goodrho_acc /= (double)goodrho_count;
  if (VERBOSE) printf("Global percentage of good regression points: %f %%\n", 100.*goodrho_acc);
 }
...
//   Argument HMIN. Type 2: Float
    ...
    strcpy(line[in].argname,"-hmin");
    strcpy(line[in].valname,"hmin");
    sprintf(line[in].explain," %s : %s  %0.2f\n",line[in].argname,
     "Multifractal variable. Minimum allowed exponent. Default:",HMIN);
    line[in].type=2;
    line[in].var_f=&HMIN;
    line[in].minvar_f=-1000.;
    line[in].maxvar_f=1000.;
    in++;

//   Argument HMAX. Type 2: Float

    strcpy(line[in].argname,"-hmax");
    strcpy(line[in].valname,"hmax");
    sprintf(line[in].explain," %s : %s  %0.2f\n",line[in].argname,
...
//   Argument DER_MODE. Type 1: Integer
    ...
    strcpy(line[in].argname,"-der_mode");
    strcpy(line[in].valname,"mode_index");
    sprintf(line[in].explain,"%s\n %s : %s %d\n",
     "\nDERIVATIVE VARIABLES\n===================",line[in].argname,
     "Mode to calculate the derivative:\n   0: Half-pixel centered difference.\n   1: Direct finite difference\n Default:",DER_MODE);
    line[in].type=1;
    line[in].var_i=&DER_MODE;
    line[in].minvar_i=0;
    line[in].maxvar_i=1;
    in++;
...
//   Argument WVINDEX. Type 1: Integer
    ...
    strcpy(line[in].argname,"-wav");
    strcpy(line[in].valname,"wav_index");
    sprintf(line[in].explain,"%s\n %s : %s %d\n",
     "\nWAVELET VARIABLES\n=================",line[in].argname,
     "Wavelet to be used.\n   0: Gaussian\n   1: Lorentzian at exponent 0.5\n   2: Lorentzian\n   3: Lorentzian at 1.5.\n Default:",WVINDEX);
    line[in].type=1;
    line[in].var_i=&WVINDEX;
    line[in].minvar_i=0;
    line[in].maxvar_i=3;
    in++;
...
//   Argument ORD_DER. Type 1: Integer
    ...
    strcpy(line[in].argname,"-der");
    strcpy(line[in].valname,"deriv_order");
    sprintf(line[in].explain," %s : %s %d\n",line[in].argname,
     "Derivative order for the wavelet. Default: ",ORD_DER);
    line[in].type=1;
    line[in].var_i=&ORD_DER;
    line[in].minvar_i=0;
    line[in].maxvar_i=2;
    in++;
...
//   Argument WAV_RANGE. Type 2: Float
    ...
    strcpy(line[in].argname,"-range");
    strcpy(line[in].valname,"scale_range");
    sprintf(line[in].explain," %s : %s %f\n",line[in].argname,
     "Range of scales in wavelet analysis. Default:",WAV_RANGE);
    line[in].type=2;
    line[in].var_f=&WAV_RANGE;
    line[in].minvar_f=1.00001;
    line[in].maxvar_f=10000.;
    in++;
 ...
 for (lar=1,flagv=1; (lar<argc)&&(flagv==1); lar++)
 {
  ...
  if (flagv==1)
  {
   ...
   else if (line[in].type%3==2) // Float type
   {
    ...
    if ((*(line[in].var_f)<line[in].minvar_f)||(*(line[in].var_f)>line[in].maxvar_f))
    {
   ...
   else if (line[in].type%3==1) // Integer type
   {
    ...
    if ((*(line[in].var_i)<line[in].minvar_i)||(*(line[in].var_i)>line[in].maxvar_i))
    {
   ...
   {
    *(line[in].flag)=1;
    if (line[in].type==3) *(line[in].var_i)=1; // Flagged flag type (3) uses var_i to store the parent flag
   }
  }
 }
...
double linear_fit( int N, double *x, double *y, double *a, double *b, double *corr)
/*
   PURPOSE:  Least square linear regresssion of y with respect to x
*/
{
   double sumx,sumy,sx,sy,sxy;
   int i;
   ...
   if (sx>NUMERICAL_RESOLUTION)
   {
    *a=sxy/(sx*sx);
    *b=sumy-*a*sumx;
    if (sy>NUMERICAL_RESOLUTION) *corr = sxy/(sx*sy);
    else                         *corr = 1.;
   }
   else
   {
...
/* Exiting by refitting */

   return(fit_matrix(data));
}

int destroy_matrix( Matrix *data)
/*
   PURPOSE: To release memory associated to a matrix
*/
{
   if ( (data->p==NULL) || (data->dimx<1) || (data->dimy<1) ) return(-1);
   ...
   free(data->p);
   data->p=NULL;
   data->dimx=data->dimy=0;
   strcpy(data->Name,""); // The compiler might complaint here. It is OK.
...
*/
{
   ...
   status=create_matrix(a.dimx,a.dimy,(a.pm!=NULL)?1:0,b);
   if (status) return(status);

   status=assign_matrix(a,*b);
...
/* Early checks */
   ...
   if (fabs(block-1.)<NUMERICAL_RESOLUTION) return(0); // For tiny res. changes do nothing!
...
*/
{
   ...
   if (output.Reader==NULL)      // not yet found...
    output=ping_ascii(filename); // looking for ASCII multicolumn 1D
...
int FFT( Matrix functionR, Matrix functionI, int sign)
/*
   PURPOSE: To calculate Fourier transform by a fast algorithm. This version
   used the decomposition in prime factors of the dimensions dimx and dimy of
   the analyzed data (provided as two matrices, one for the real part and
   another for the imaginary part). For that reason, no requirement dimx=2^n
   is imposed. Notice however that if some prime factors are large, the routine
...
*/
{
   ...
   if ((functionR.dimx!=functionI.dimx)||(functionR.dimy!=functionI.dimy)) return(-1);

   status=FFThorizontal(functionR,functionI,sign);
   if (functionR.dimy>1) status+=FFTvertical(functionR,functionI,sign);
...
*/
{
   Matrix workR=_Matrix,workI=_Matrix;
   ...
   create_matrix(functionR.dimx,functionR.dimy,0,&workR);
   create_matrix(functionI.dimx,functionI.dimy,0,&workI);
   ...
   for (iy=0;iy<dimy;iy++)
   {
    ...
    while (dimb>1)
    {
     ...
     if (inu==1) PFFThorizontal(dima,dimb,dimc,iy,functionR.Signal,functionI.Signal,workR.Signal,workI.Signal,sign);
     else        PFFThorizontal(dima,dimb,dimc,iy,workR.Signal,workI.Signal,functionR.Signal,functionI.Signal,sign);
    ...
    if (inu==0)
    {
     for (ix=0;ix<dimx;ix++)
     {
      functionR.Signal[iy][ix]=workR.Signal[iy][ix];
      functionI.Signal[iy][ix]=workI.Signal[iy][ix];
     }
    }

    for (ix=0;ix<dimx;ix++)
    {
     functionR.Signal[iy][ix]/=sqrt((double)dimx);
     functionI.Signal[iy][ix]/=sqrt((double)dimx);
    }
   }

   destroy_matrix(&workR);
   destroy_matrix(&workI);
...
*/
{
   ...
   for (ib=0;ib<dimb;ib++)
   {
    ...
    for (jcr=0;jcr<dimc;jcr++)
    {
     ...
     auxR=omegaR*sumR-omegaI*sumI;
     auxI=omegaR*sumI+omegaI*sumR;
     sumR=auxR+uinR[iy][ix];
     sumI=auxI+uinI[iy][ix];
    }
    ix=ib+dimb*(ia+ic*dima);
    uoutR[iy][ix]=sumR;
    uoutI[iy][ix]=sumI;
   }
    auxR=omegaR*deltaR-omegaI*deltaI;
    auxI=omegaR*deltaI+omegaI*deltaR;
    omegaR=auxR;
    omegaI=auxI;
   }
   }
...
*/
{
   ...
   for (ix=0;ix<dimx;ix++)
   {
    ...
    while (dimb>1)
    {
     ...
     if (inu==1) PFFTvertical(dima,dimb,dimc,ix,functionR.Signal,functionI.Signal,workR.Signal,workI.Signal,sign);
     else        PFFTvertical(dima,dimb,dimc,ix,workR.Signal,workI.Signal,functionR.Signal,functionI.Signal,sign);
    ...
    for (iy=0;iy<dimy;iy++)
    {
     functionR.Signal[iy][ix]/=sqrt((double)dimy);
     functionI.Signal[iy][ix]/=sqrt((double)dimy);
    }
   }
...
int reconstruct( Matrix gx, Matrix gy)
/*
   EASTER EGG: A routine performing reconstruction from gradient, as in
   Turiel and del Pozo, IEEE Trans. Im. Proc. (2002). This is something
   that you could find useful, and it is placed here as a gift for
   rewarding your patience in reading all this long code.
*/
/*
   PURPOSE: Common access point to perform gradient reconstruction
*/
{
   int status;
   ...
   {
    case 0:
     status=reconstruct_FFT(gx,gy);
     break;
    case 1:
     status=reconstruct_naif(gx,gy);
     break;
    default:
     printf("Unrecognized derivative mode\n");
     return(-1);
...
int reconstruct_naif( Matrix gx, Matrix gy)
{
   ...
   for (iy=0;iy<gx.dimy;iy++)
   {
      ...
      for (ix=0;ix<gx.dimx;ix++)
      {
         ...
         if (modx+mody > NUMERICAL_RESOLUTION)
         {
            auxR=(dxR*gx.Signal[iy][ix]-dxI*gxI.Signal[iy][ix]+dyR*gy.Signal[iy][ix]-dyI*gyI.Signal[iy][ix])/(modx+mody);
            auxI=(dxR*gxI.Signal[iy][ix]+dxI*gx.Signal[iy][ix]+dyR*gyI.Signal[iy][ix]+dyI*gy.Signal[iy][ix])/(modx+mody);
         }
         else
         {
            auxR=0.;
...
int exponent_filter( double expon, Matrix data)
{
   ...
   for (ix=0;ix<data.dimx;ix++)
   {
      ...
      x=2.*sin(PI*x);
      f=sqrt(x*x+y*y);
      if (f>NUMERICAL_RESOLUTION) f=pow(f,expon);
      else                        f=0.;
      auxR.Signal[iy][ix]*=f;
      auxI.Signal[iy][ix]*=f;
   }
...
/* Early checks */
   ...
   if ( (dimx<1)||(dimy<1)||(sc<NUMERICAL_RESOLUTION) ) return(-1);
...
/* Normalizing directly by mean and logarithmically by scale */
      ...
      for (ix=0; ix<data.dimx; ix++)
      {
         ...
         if (nomask)
         {
            if (fabs(dmasa.Signal[iy][ix]/mean) > NUMERICAL_RESOLUTION)
               expon->Signal[iy][ix] = log(fabs(dmasa.Signal[iy][ix])/mean) / log(output.sc0);
            else
               expon->Signal[iy][ix] = log(NUMERICAL_RESOLUTION)            / log(output.sc0);
      ...
      for (ix=0; ix<data.dimx; ix++)
      {
         ...
         if (nomask)
         {
            ...
            for (ip=0;ip<WVPOINTS;ip++)
            {
               ...
               if (wp>NUMERICAL_RESOLUTION) yy[ip]=log(wp);
               else yy[ip]=log(NUMERICAL_RESOLUTION);
               yy[ip]*=xx[ip];
            }
...
/*
   ...
   Explanation: We define "translational invariance" as the scale independence of the first order moment,
   i.e., the expected value, of the normalized multifractal measure: <T_r> ~ r^\tau_1 with \tau_1 = 0,
   which is a very reasonable requirement for any physical signal.
   This is explicitly assumed in the punctual mode, when we have divided the T_r(x) by its mean to ensure
   that the prefactor a(x) in the multifractal scaling, T_r(x) = a(x) r^h(x) + o(x), has small influence, also
   implicitly assumed in the multiscale regression mode, when wavelet projections are normalized.
   On the other hand, the \tau_p functional is the Legendre-transform of the reduced singularity spectrum D(h),
   i.e., \tau_p = {ph - D(h)} so \tau_1 = {h - D(h)} = 0 implies that the spectrum is tangent to the f(h)=h line.
   However, numerical deviations can be expected, since numerically computed means are not always translational
   invariant. If this is the case, we adapt the estimated spectrum to satisfy this property.
*/
   {
    ihmode = 0;
...
/* First step: vectors are cleaned up */
   ...
   for (ih=0;ih<NBOX;ih++) histo[ih]=val[ih]=0.;
...
/* Second step: Data are explored, taking the mask into account if required */
   ...
   for (ix=0;ix<data.dimx;ix++)
   {
    ...
    if (nomask)
    {
     ih=(int)((data.Signal[iy][ix]-min)/d);
     if ( (ih>=0)&&(ih<NBOX) ) // if ih is in the appropriate range
     {
      val[ih]   += data.Signal[iy][ix]; // it contributes to the mean binned val
      histo[ih] += 1.;
     }
     else if (ih==NBOX) // closing for the max
     {
      val[NBOX-1]   += data.Signal[iy][ix];
      histo[NBOX-1] += 1.;
     }
    }
   }
...
/*
   Structure of the file: first column: uniform representative of val
                         second column: average representative of val
                          third column: number of cases
*/
...
void histoh_adapt( double sc0, double sc1, double *h, double *histoh)
/*
   PURPOSE: To adapt singularity histograms coming from data with different resolutions
...
*/
   ...
   for (ip=0;ip<Nh;ip++)
   {
    ...
    if (prob>NUMERICAL_RESOLUTION) dp=Ks*sqrt((1.-prob)/(prob*Nev)); // Ks sigmas in the prob. distribution
    else dp=0.;
/*
...
/* Finding and normalizing by the mode */
   ...
   if (maxprob>NUMERICAL_RESOLUTION) for(ip=0;ip<Nh;ip++) histo_r[ip]/=maxprob;

/* Evaluating experimental reduced D(h) (red{D(h)} = D(h)-D_space */

   for (ip=0;ip<Nh;ip++)
   {
    if (histo_r[ip]>NUMERICAL_RESOLUTION) Dh[ip]=-log(histo_r[ip])/log(sc0);
    else Dh[ip]=-log(NUMERICAL_RESOLUTION)/log(sc0);
   }
